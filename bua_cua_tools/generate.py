...
- 在 `ctx.withRecovery` 的 primary 中，codegen 派生的关键 Playwright UI 小动作必须用 `await ctx.action("动作名", async () => {{ ... }})` 保留动作边界；`ctx.action` 是 runtime API，不是模型自定义 helper。
- 模型不需要判断哪些动作容易触发弹窗；只需要保留填写、点击、选择、触发下载等关键动作边界。高风险业务确认动作（Submit/Apply/Confirm/Delete/Pay/Download/Save 等）不要使用自动重试型 `ctx.action`，除非显式传入更保守的 options 或禁用 interruption。
